...
function TeamForm({ initial, onSave, onCancel, loading }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSave)} className="space-y-5">
      <div>
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Member Information</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-semibold text-gray-600 block mb-1.5">Full Name <span className="text-red-400">*</span></label>
          ...
          <div>
            ...
            <input {...register('role', { required: true })} placeholder="Frontend Developer" className={inputCls} />
          </div>
          <div>
      ...
      <div>
        <label className="text-xs font-semibold text-gray-600 block mb-1.5">Bio <span className="text-gray-400 font-normal">(optional)</span></label>
        <textarea rows={2} {...register('bio')} className={`${inputCls} resize-none`} placeholder="Brief bio about this team member..." />
      </div>
...
export default function AdminTeamPage() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-team'] }); setEditing(null) },
  })
