    ...
    <form onSubmit={handleSubmit(onSave)} className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input {...register('role', { required: true })} placeholder="Frontend Developer"
            className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg
              focus:outline-none focus:ring-2 focus:ring-brand-blue/30 focus:border-brand-blue" />
        </div>
        <div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Manage your About page team section.</p>
          </div>
        ...
        {showForm && (
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <h3 className="font-heading text-base font-semibold text-gray-800 mb-4">New Team Member</h3>
            <TeamForm
        ...
        <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
          {isLoading ? (
            ...
            <p className="text-center py-10 text-gray-400 text-sm">No team members yet.</p>
          ) : members.map(m => (
            <div key={m.id}>
              {editing?.id === m.id ? (
                <div className="p-5">
                ...
                <div className="p-4 flex items-center gap-4">
                  {m.photoUrl ? (
                    ...
                    <div className="w-10 h-10 rounded-full bg-brand-blue/10 flex items-center
                      justify-center shrink-0 font-bold text-brand-blue text-sm">
                      {m.name[0]}
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-gray-900 text-sm">{m.name}</p>
                    <p className="text-xs text-brand-red">{m.role}</p>
                  </div>
