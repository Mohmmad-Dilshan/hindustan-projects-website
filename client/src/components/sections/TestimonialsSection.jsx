...
export default function TestimonialsSection() {
  ...
  const initials = t ? t.name.split(' ').map(n => n[0]).slice(0, 2).join('').toUpperCase() : ''
  ...
  return (
    <section className="py-20 bg-white relative overflow-hidden" aria-labelledby="testimonials-heading">
      ...
      <Container>
        <motion.div initial="hidden" whileInView="visible" viewport={viewportOnce} variants={fadeUp} className="text-center mb-12">
          <SectionHeading
          ...
          {isLoading ? (
            ...
            <div className="relative">
              ...
              <Card className="bg-gray-50/50 backdrop-blur-sm border border-gray-100 rounded-3xl p-8 sm:p-14 shadow-lg shadow-gray-100/40 relative overflow-hidden">
                ...
                <Quote className="absolute right-8 top-8 w-24 h-24 text-brand-blue/4 pointer-events-none" strokeWidth={1} />
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3.5 border-t border-gray-200/60 pt-6 w-full max-w-md justify-center">
                      {t.avatarUrl ? (
                        <img src={t.avatarUrl} alt={t.name}
                          className="w-12 h-12 rounded-full object-cover shrink-0 border-2 border-white shadow-md" loading="lazy" />
                      ) : (
                        <div className="w-12 h-12 rounded-full bg-brand-blue flex items-center
                          justify-center font-heading text-sm font-bold text-white shrink-0 border-2 border-white shadow-md">
                          {initials}
                        </div>
                      )}
                      <div className="text-center sm:text-left">
                        <h4 className="font-heading text-base font-bold text-brand-blue leading-none mb-1">{t.name}</h4>
