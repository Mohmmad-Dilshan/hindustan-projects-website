    ...
    <form onSubmit={handleSubmit(onSave)} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="sm:col-span-2">
          <label className="text-xs font-semibold text-gray-600 block mb-1.5">Company Name <span className="text-red-400">*</span></label>
          <input {...register('name', { required: true })} placeholder="Bhilwara Textiles" className={inputCls} />
        </div>
        <div>
      ...
      <div>
        <label className="text-xs font-semibold text-gray-600 block mb-1.5">Logo URL <span className="text-gray-400 font-normal">(optional)</span></label>
        <input {...register('logoUrl')} placeholder="https://..." className={inputCls} />
      </div>
      <div className="flex items-center gap-2.5">
        <input type="checkbox" id="pActive" {...register('isActive')}
          className="w-4 h-4 rounded border-gray-300 text-brand-blue focus:ring-brand-blue/25 cursor-pointer" />
        <label htmlFor="pActive" className="text-sm text-gray-600 cursor-pointer">Show on website</label>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">Client logos shown in the "Trusted By" banner on the homepage.</p>
            </div>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-white border border-gray-100 rounded-xl py-16 flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-gray-400">Add client logos to display in the Trusted By section.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {items.map((p, idx) => (
