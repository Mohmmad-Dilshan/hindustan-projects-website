...
import {
  ...
  ShieldAlert, KeyRound, Mail, ShieldCheck, Info, Copy, Check,
} from 'lucide-react'
    ...
    <div className="bg-white border border-gray-100 border-l-4 border-l-brand-blue rounded-xl p-6 shadow-sm">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
        <div>
          ...
          {errors.newEmail && (
            <p className="text-xs text-red-500 mt-1.5 flex items-center gap-1">
              <AlertCircle className="w-3 h-3" />{errors.newEmail.message}
            </p>
          )}
        </div>
    ...
    <div className="bg-white border border-gray-100 border-l-4 border-l-brand-blue rounded-xl p-6 shadow-sm">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
        <PasswordInput label="Current Password" name="currentPassword"
          ...
          show={show.curr} onToggle={() => setShow(s => ({ ...s, curr: !s.curr }))} />
        <PasswordInput label="New Password (min 8 characters)" name="newPassword"
          register={register} error={errors.newPassword}
          show={show.new} onToggle={() => setShow(s => ({ ...s, new: !s.new }))} />
        <PasswordInput label="Confirm New Password" name="confirmPassword"
          register={register} error={errors.confirmPassword}
          show={show.conf} onToggle={() => setShow(s => ({ ...s, conf: !s.conf }))} />
  ...
  return (
    <div className="bg-white border border-orange-100 border-l-4 border-l-orange-400 rounded-xl p-6 shadow-sm">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h2 className="font-heading text-base font-semibold text-gray-800">Integration Page Master Key</h2>
      </div>
      <p className="text-xs text-gray-500 mb-4 leading-relaxed">
        This key unlocks the Integration Settings page. It protects your Cloudinary,
        database, and email credentials. Change it regularly and keep it secret.
      </p>
      ...
      <div className="mb-5 bg-orange-50 border border-orange-200 rounded-xl p-4">
        ...
        {keyLoading ? (
          ...
          <div className="flex items-center gap-2">
            <code className={`flex-1 text-sm font-mono bg-white border border-orange-200 rounded-lg px-3 py-2
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
        <PasswordInput label="Current Master Key" name="currentKey"
          register={register} error={errors.currentKey}
          show={show.curr} onToggle={() => setShow(s => ({ ...s, curr: !s.curr }))} />
        <PasswordInput label="New Master Key (min 8 characters)" name="newKey"
          register={register} error={errors.newKey}
          show={show.new} onToggle={() => setShow(s => ({ ...s, new: !s.new }))} />
        <PasswordInput label="Confirm New Master Key" name="confirmKey"
          register={register} error={errors.confirmKey}
          show={show.conf} onToggle={() => setShow(s => ({ ...s, conf: !s.conf }))} />
      ...
      <div className="space-y-6 max-w-lg">
        ...
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm p-5">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-brand-blue to-brand-blue/70
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                {admin?.role === 'SUPER_ADMIN' ? 'Super Admin' : 'Administrator'} · Hindustan Projects
              </p>
              <span className="inline-flex items-center gap-1 mt-1.5 text-xs px-2 py-0.5
